...
var hui = (function(selector, undefined){
	...
	huibase.plusReady = function(callBack){document.addEventListener('plusready', function(){if(callBack){callBack();}});};
	huibase.ajax = function(url, success, err, sets){
		...
		if(sets.timeout){xhr.timeout = sets.timeout;}
	...
	huibase.open = function(winName, styles, isClose, extras){var w = this.create(winName, styles, isClose, extras); plus.webview.show(w, 'slide-in-right'); return w;};
	...
	huibase.Back   = function(){
		...
		if(selfW.id == plus.runtime.appid || selfW.id == 'HBuilder'){
    		if(huibackNum < 1){hui.toast('再按一次退出程序'); huibackNum++; setTimeout(function(){huibackNum = 0;},3000);}else{selfW.close();}
		}else{
	...
	huibase.toast = function(msg){plus.nativeUI.toast(msg);};
	...
	huibase.refresher = null;
	huibase.refresh = function(callBack,sets){
		if(!sets){sets = {height:'50px', range:"80px", 'contentdown':"继续下拉进行刷新", contentover:"释放立即刷新", contentrefresh:"正在刷新..."};}
		this.refresher = plus.webview.currentWebview();
		this.refresher.setPullToRefresh({support:true, height:sets.height, range:sets.range, contentdown:{caption:sets.contentdown}, contentover:{caption:sets.contentover}, contentrefresh:{caption:sets.contentrefresh}},callBack);
	};
	huibase.endRefresh = function(){if(this.refresher){this.refresher.endPullToRefresh();}}
	huibase.isLoadMoreIng = false;
	huibase.loadMore = function(callBack, sets){
		huibase.loadMoreCallBack = callBack;
		if(!sets){huibase.loadMoreSets = {endContent:"已经加载全部"};}
		huibase.LoadMoreDom = document.getElementById('HUI_LoadMoreLoading');
		if(!huibase.LoadMoreDom){
			huibase.LoadMoreDom = document.createElement('div');
			huibase.LoadMoreDom.setAttribute('id', 'HUI_LoadMoreLoading');
			document.body.appendChild(huibase.LoadMoreDom);
		}
		document.addEventListener("plusscrollbottom", huibase.loadMoreFun, false);
	};
	huibase.loadMoreFun = function(){
		if(huibase.isLoadMoreIng){return false;}
		huibase.isLoadMoreIng = true;
		huibase.LoadMoreDom.innerHTML = '<div id="HUI_LoadMoreLoadingIn"><div></div><div></div><div></div><div></div><div></div></div>';
		huibase.loadMoreCallBack();
	};
	huibase.endLoadMore = function(isLast){
		if(!isLast){huibase.isLoadMoreIng = false; huibase.LoadMoreDom.innerHTML = '';}else{huibase.isLoadMoreIng = true; huibase.LoadMoreDom.innerHTML = this.loadMoreSets.endContent;}
	};
	huibase.removeLoadMore = function(){document.removeEventListener("plusscrollbottom", this.loadMoreFun, false);}
